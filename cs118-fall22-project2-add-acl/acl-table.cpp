...
ACLTable::lookup(uint32_t srcIp, uint32_t dstIp, uint8_t protocol, uint16_t srcPort, uint16_t dstPort) const
{
  ...
  for (std::list<ACLTableEntry>::const_iterator rule = m_entries.begin(); rule != m_entries.end(); rule++) {
    ...
    if ((rule->src & rule->srcMask) != (srcIp & rule->srcMask) || (rule->dest & rule->destMask) != (dstIp & rule->destMask) || (rule->protocol * rule->protocolMask) != (protocol & rule->protocolMask) || (rule->srcPort & rule->srcPortMask) != (srcPort & rule->srcPortMask) || (rule->destPort & rule->destPortMask) != (dstPort & rule->destPortMask)) {
      continue;
    }
